...
void compute_expsum(const int count, fifo<chunk_t>& ins, fifo<float>& outs) {
...
void compute_sum(const int count, fifo<chunk_t>& ins, fifo<float>& outs) {
	for (int i = 0; i < count; i++) {
#pragma HLS pipeline
		chunk_t buf = ins.read();
...
#pragma HLS dataflow
	...
	compute_sum(count, insm, outsm);
	compute_expsum(count, insv, outsv);
