...
using uint_t = ap_uint<32>;

// @thanks https://acri-vhls-challenge.web.app/user/@Ryuz88/code/bF8U1WQa7LxBNKl0HXzf
ap_uint<8> increase_if_not_zero(const ap_uint<8> i) {
	static const ap_uint<8> table[] = {
...
// @thanks https://acri-vhls-challenge.web.app/user/@Ryuz88/code/Tjm6v24lejtLlJltEoab
uint_t f2i(const float f) {
#pragma HLS inline
	return *(unsigned int*)&f;
}

float i2f(const uint_t i) {
#pragma HLS inline
	return *(float*)&i;
}

uint_t bai(const uint_t v) {
#pragma HLS inline
	v.range(30, 23) = v.range(30, 23) + 1;
	//v.range(30, 23) = increase_if_not_zero(v.range(30, 23));
	return v;
}
...
#pragma HLS unroll factor=DEGREE skip_exit_check
	// @see ERROR: [HLS 214-341] in function 'kernel.6': Pointer cast is not supported. Cannot apply array transformation pragma/directive.
