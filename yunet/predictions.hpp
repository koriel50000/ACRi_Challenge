...
private:
    static constexpr int NUM_PRED_TYPES = 4; // cls, bbox, obj, kps
    static constexpr int NUM_STRIDES = 3;
    static constexpr int STRIDES[] = { 8, 16, 32 };
    static constexpr int NUM_QLEVELS_4BIT = 16;

    static constexpr int NUM_PRED_CELLS = 20*20 + 10*10 + 5*5; // [(20, 20), (10, 10), (5, 5)]
...
public:
    void reset() {
#pragma HLS bind_storage variable=preds type=ram_1p
#pragma HLS array_partition variable=preds cyclic factor=NUM_PRED_FEATURES dim=2
#pragma HLS aggregate variable=detects
#pragma HLS array_partition variable=FEATURE_DECODE_TABLE block factor=NUM_STRIDES dim=1
#pragma HLS array_partition variable=FEATURE_OFFSET complete
#pragma HLS array_partition variable=STRIDES complete
        pred_ptr = 0;
        feat_index = 0;
    }
    ...
    const Detection& get_detection(int i) const {
        return detects[i];
    }
...
#pragma HLS pipeline
                    ...
                    if (score & 0x8000) { // score >= 32768(=0.5)
                        ...
                        uint8_t half_w = bboxexp[pred[3]] / 2;
                        uint8_t half_h = bboxexp[pred[4]] / 2;
                        detect->x1 = cx - half_w;
                        detect->y1 = cy - half_h;
                        detect->x2 = cx + half_w;
                        detect->y2 = cy + half_h;
