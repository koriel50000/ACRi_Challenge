...
void sha256(const block_t& in, hash_t& H) {
	hash_t A[65];
	A[0] = H; 
	...
	W[16] = W[0] + sigma0(W[1]);
	W[17] = W[9] + sigma1(W[14]);
	uint32_t B = A[0][7] + K[0] + W[0];

	for (int j = 0; j < 64; j++) {
#pragma HLS pipeline II=1
		uint32_t t1 = B + SIGMA1(A[j][4]) + Ch(A[j][4], A[j][5], A[j][6]);
		uint32_t t2 = SIGMA0(A[j][0]) + Maj(A[j][0], A[j][1], A[j][2]);
		uint32_t w1 = W[9 + 1] + sigma1(W[14 + 1]);
		uint32_t w0 = W[0 + 1] + sigma0(W[1 + 1]);
		B = A[j][6] + K[j + 1] + W[1];
